...
public class SecurityConfig {
  ...
  @Bean ReactiveJwtDecoder keycloakOidcReactiveJwtDecoder(
      ...
      @Value("${keycloak.relative-path:}") String relativePath) {
    String issuerUri = KeycloakIssuerUriResolver.resolveEffectiveIssuerUri(
        null, null, baseUrl, relativePath, realmName); // 명시 issuer-uri가 있으면 1번째 인자로 전달
    NimbusReactiveJwtDecoder decoder =
        NimbusReactiveJwtDecoder.withJwkSetUri(KeycloakIssuerUriResolver.resolveJwkSetUri(issuerUri)).build();
  ...
  SecurityWebFilterChain chain(ServerHttpSecurity http, KeycloakClient kc,
      ...
      ReactiveJwtDecoder keycloakOidcReactiveJwtDecoder) {

    var authManager = new KeycloakReactiveAuthenticationManager(kc, clientId, keycloakOidcReactiveJwtDecoder);
    authManager.setRoleMapping(props.getRoleMapping()); // 보안 Advisory 7 — 미호출 시 기본값(SEPARATE_NAMESPACE) 그대로 사용됨
