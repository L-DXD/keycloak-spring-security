...
feat(core): JWT-Principal 변환기 구현 및 설계 문서 추가" -m "Keycloak Access Token을 Spring Security의 Authentication 객체로 변환하는 기능을 구현하고, 관련 설계 문서를 추가합니다.

주요 변경 사항:
  - `KeycloakJwtAuthenticationConverter`를 구현하여 JWT 클레임에서 사용자 이름과 역할 정보를 추출.
      - `realm_access` 및 `resource_access`의 역할을 조합하여 `GrantedAuthority`로 매핑.
      - 역할 목록의 `null` 값을 안전하게 필터링.
  - `core` 모듈의 `build.gradle`에 `spring-boot-starter-oauth2-resource-server` 의존성을 추가하여 컴파일 문제를 해결.
  - 성공, 실패, 바운더리 케이스를 포함한 단위 테스트를 작성하여 기능의 안정성을 검증.
  - 토큰 변환 및 인증 흐름 재정의에 대한 상세 계획을 담은 설계 문서(`docs/05-tokeninfo-to-principal-mapper.md`)를 추가.