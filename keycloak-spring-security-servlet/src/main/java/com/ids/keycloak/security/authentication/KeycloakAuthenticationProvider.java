...
import com.ids.keycloak.security.exception.AuthenticationRequiredException;
...
public class KeycloakAuthenticationProvider implements AuthenticationProvider {
   ...
   private Authentication authenticateWithRefreshToken(KeycloakAuthentication authRequest) {
      ...
      if (!(authRequest.getDetails() instanceof OAuth2AuthorizedClient existingClient)) {
         throw new ConfigurationException("인증 요청에 OAuth2AuthorizedClient 정보가 없습니다.");
      }

      if (existingClient.getRefreshToken() == null) {
         throw new RefreshTokenException();
      }

      KeycloakTokenInfo newTokens = refreshTokens(existingClient.getRefreshToken().getTokenValue());
   ...
   private Authentication createAuthenticatedToken(Jwt validatedIdToken, Jwt validatedAccessToken, KeycloakTokenInfo newTokens) {
      ...
      KeycloakAuthentication authenticatedToken = new KeycloakAuthentication(principal, validatedIdToken.getTokenValue(),
